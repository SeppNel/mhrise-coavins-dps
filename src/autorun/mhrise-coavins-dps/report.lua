...
this.sumDamageSourcesList = function(sources)
	...
	for _,source in pairs(sources) do
		local c = DATA.sumDamageCountersList(source.counters)
		sum.total = sum.total + c.total
		sum.physical  = sum.physical  + c.physical
		sum.elemental = sum.elemental + c.elemental
		sum.condition = sum.condition + c.condition
		sum.poison    = sum.poison    + c.poison
		sum.blast     = sum.blast     + c.blast
		sum.otomo     = sum.otomo     + c.otomo

		sum.numHit    = sum.numHit + c.numHit
		sum.maxHit     = math.max(sum.maxHit, c.maxHit)
		sum.numUpCrit = sum.numUpCrit + c.numUpCrit
		sum.numDnCrit = sum.numDnCrit + c.numDnCrit
	end
...
this.mergeBossIntoReport = function(report, boss)
	...
	for _,source in pairs(boss.damageSources) do
		...
		or (not CORE.attackerIdIsOtomo(effSourceId) and STATE._FILTERS.INCLUDE_OTHER)
		then
			-- get report item, creating it if necessary
			local item = this.getReportItem(report, effSourceId)
			if not item then
				item = this.initializeReportItem(effSourceId)
