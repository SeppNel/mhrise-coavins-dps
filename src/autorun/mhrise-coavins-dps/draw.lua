...
this.drawRichText = function(text, x, y, colorText, colorShadow)
	if CORE.CFG('TEXT_DRAW_SHADOWS') and colorShadow then
		...
		d2d.text(STATE.FONT, text, x + offsetX, y + offsetY, colorShadow)
	end
	d2d.text(STATE.FONT, text, x, y, colorText)
end
...
this.drawRichDamageBar = function(item, x, y, maxWidth, h, colorPhysical, colorElemental)
	...
	if not CORE.CFG('DRAW_BAR_USE_UNIQUE_COLORS') then
...
this.drawReportItem = function(item, x, y, width, height)
	...
	if CORE.CFG('USE_MINIMAL_BARS') then
		...
		-- hr
		if item.playerNumber and item.rank and CORE.CFG('DRAW_BAR_REVEAL_HR') then
		...
		if CORE.CFG('DRAW_TABLE_BACKGROUND') then
			-- draw background
			d2d.fill_rect(x, y, width, height, CORE.COLOR('BAR_BG'))
		end
	...
	if CORE.CFG('DRAW_BAR_OUTLINES') then
		-- draw outline
		d2d.outline_rect(x, y, width, height, 2, CORE.COLOR('BAR_OUTLINE'))
	end
end
...
this.drawReport = function(index)
	...
	if CORE.CFG('DRAW_TITLE') then
		...
		if CORE.CFG('DRAW_TITLE_BACKGROUND') then
			-- title background
			d2d.fill_rect(origin_x, origin_y, tableWidth, titleHeight, CORE.COLOR('TITLE_BG'))
		end
		...
		if CORE.CFG('DRAW_TITLE_TEXT') then
			...
			-- get quest duration
			local timeMinutes = STATE.MANAGER.QUEST:call("getQuestElapsedTimeMin")
			local timeSeconds = STATE.MANAGER.QUEST:call("getQuestElapsedTimeSec")
	...
	if CORE.CFG('DRAW_HEADER') then
		...
		if CORE.CFG('DRAW_HEADER_BACKGROUND') then
			-- background
			d2d.fill_rect(origin_x, y, tableWidth, headerHeight, CORE.COLOR('TITLE_BG'))
		end

		if CORE.CFG('DRAW_BAR_COLORBLOCK') and CORE.CFG('DRAW_BAR_REVEAL_HR') then
			this.drawRichText('HR', x, y, CORE.COLOR('GRAY'), CORE.COLOR('BLACK'))
		end
	...
	if #report.items == 0 then
		...
		d2d.text(STATE.FONT, 'No data', x, y, CORE.COLOR('GRAY'))
	end
...
-- main draw function
this.d2dDraw = function()
	if STATE.DRAW_WINDOW_SETTINGS -- always draw overlay if settings window is open
	or (STATE.DRAW_OVERLAY and -- draw in one of the following circumstances if overlay is enabled
	   (DATA.isInTestMode() or STATE.IS_IN_QUEST or STATE.IS_IN_TRAININGHALL)) then
